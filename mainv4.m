...
if isempty(imdb.encoder)
	...
	for i = 1:imdb.batchSize:numel(imdb.Sets{1}.imagePath)
	        tic
	        encode{cnt} = extractFeature(imdb,i,'mode','train');
		cnt=cnt+1;
	        toc
	end
	encode=cat(2,encode{:});
	...
	imdb.encoder.eigvector = eigvector;
	imdb.encoder.means = m;
	keyboard
    	save(imdb.encoderPath, 'encoder');
    	disp(['encoder created and saved to ',imdb.encoderPath]);
...
switch imdb.feature
    ...
    case 'dcnn'
        out.feature = get_dcnn_features_modified(imdb.net,bachGround_patch,'encoder',imdb.encoder.fisher);
    case 'dsift'
