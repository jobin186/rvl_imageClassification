...
dataHomeLoc = ['/users/jobinkv/DSSE-200-release']; % data path
dfolder_locNo = 4;
...
imdb.expDir = ['/users/jobinkv/dssc_models/',imdb.feature]; % 9--> 1:8k,10-->1:end
opts.useGpu = true;
...
imdb.dataLoc =['/tmp/',dfoldr];
imdb = get_database(imdb);
imdb.resultPath = fullfile(imdb.expDir, sprintf('results-%s.mat', imdb.feature,imdb.stride,imdb.patchSize)) ;
imdb.encoderPath = fullfile(imdb.modelDir, sprintf('encoder-%s.mat', imdb.feature,imdb.stride,imdb.patchSize)) ;
